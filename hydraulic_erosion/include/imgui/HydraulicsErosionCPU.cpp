    ...
    void simulationStep(bool isPlacingRiver) {
        ...
        for (int y = 0; y < gridSize; y++) {
            for (int x = 0; x < gridSize; x++) {
                ...
                if (enableRiver && !isPlacingRiver) {
                    ...
                    if (dist < riverRadius) d_new += dt * riverRate;
                }
                water[WRITE][i] = d_new;
            }
        }
        ...
        for (int y = 0; y < gridSize; y++) {
            for (int x = 0; x < gridSize; x++) {
                ...
                if (x > 0) f_new.x = calcFlux(x - 1, y, f_old.x); // Left
                if (x < gridSize - 1) f_new.y = calcFlux(x + 1, y, f_old.y); // Right
                if (y > 0) f_new.z = calcFlux(x, y - 1, f_old.z); // Top
                if (y < gridSize - 1) f_new.w = calcFlux(x, y + 1, f_old.w); // Bottom
                ...
                if (sum_flux > 0.0001f) {
                    ...
                    if (fluxVol > waterVol) K = std::min(1.0f, waterVol / fluxVol);
                }
                flux[i] = f_new * K;
            }
        }
        ...
        for (int y = 0; y < gridSize; y++) {
            for (int x = 0; x < gridSize; x++) {
                ...
                if (d_avg > 0.01f) {
                    ...
                    if (len > 100.0f) vel = (vel / len) * 100.0f; // MAX_VELOCITY
                }
                velocity[i] = vel;
            }
        }
