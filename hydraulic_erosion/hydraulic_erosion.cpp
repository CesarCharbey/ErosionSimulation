...
public:
    ...
    void initializeTerrain()
    {
            ...
            if (img) {
                ...
                for (int y = 0; y < GRID_SIZE; y++) {
                    for (int x = 0; x < GRID_SIZE; x++) {
                        ...
                        if (x <= 1 || x >= GRID_SIZE - 2 || y <= 1 || y >= GRID_SIZE - 2) h = 0.0f;
                        if (h < 1.0f) h = 1.0f;
        ...
        if (TERRAIN_MODE == 0) {
            ...
            auto hash = [](float n) { return std::fmod(std::sin(n) * 43758.5453f, 1.0f); };
            ...
            auto fbm = [&](float x, float y, int octaves) {
                ...
                for (int i = 0; i < octaves; ++i) { v += a * noise(x, y); x = x * 2.0f + shift; y = y * 2.0f + shift; a *= 0.5f; }
                return v;
            };
            auto ridge = [&](float x, float y, int octaves) {
                ...
                for (int i = 0; i < octaves; ++i) {
                    ...
                    n = pow(n, 2.0f); v += n * a * prev; prev = n; x = x * 2.0f + shift; y = y * 2.0f + shift; a *= 0.5f;
                }
                return v;
            };
        ...
        glBindTexture(GL_TEXTURE_2D, waterTex[0]); glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, GRID_SIZE, GRID_SIZE, GL_RED, GL_FLOAT, zeroData.data());
        glBindTexture(GL_TEXTURE_2D, waterTex[1]); glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, GRID_SIZE, GRID_SIZE, GL_RED, GL_FLOAT, zeroData.data());
        glBindTexture(GL_TEXTURE_2D, sedimentTex[0]); glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, GRID_SIZE, GRID_SIZE, GL_RED, GL_FLOAT, zeroData.data());
        glBindTexture(GL_TEXTURE_2D, sedimentTex[1]); glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, GRID_SIZE, GRID_SIZE, GL_RED, GL_FLOAT, zeroData.data());
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
...

        {
            ...
            if (ImGui::CollapsingHeader("Generation Terrain / Heightmap", ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                changed |= ImGui::RadioButton("Procedural (Bruit)", &TERRAIN_MODE, 0); ImGui::SameLine();
                ...
                if (ImGui::Button("Générer / Recharger le Terrain") || changed)
                {
                    ...
                    // On force la synchro GPU -> CPU pour que le CPU reçoive la nouvelle carte !
